...
![image-20231123100918604](https://zhangwenkang666.oss-cn-beijing.aliyuncs.com/image-20231123100918604.png)

在select中的非组函数字段 一定要写在group by中
...
**区别1：WHERE 可以直接使用表中的字段作为筛选条件，==但不能使用分组中的计算函数作为筛选条件==(where不能使用聚合函数)；HAVING 必须要与 GROUP BY 配合使用，可以把分组计算的函数和分组字段作为筛选条件（having中可以使用聚合函数）。** 
...
![image-20231123102901164](https://zhangwenkang666.oss-cn-beijing.aliyuncs.com/image-20231123102901164.png)
