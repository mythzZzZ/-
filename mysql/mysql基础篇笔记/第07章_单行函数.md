...
| ROUND(x)            | 返回一个对x的值进行四舍五入后，最接近于X的整数，有很多位小数，只对第一个小数四舍五入 |
...
| CHAR_LENGTH(s)                    | 返回字符串s的字符数。作用与CHARACTER_LENGTH(s)相同，一个字母占一个字符，一个汉字占一个字符 |
| LENGTH(s)                         | 返回字符串s的字节数，和字符集有关。一个字母占一个字节，一个汉字占3个字节 |
...
| TRIM(s1 FROM s)                   | 去掉字符串s中开始与结尾位置的s1                              |
...
| ELT(m,s1,s2,…,sn)                 | 返回指定位置的字符串，如果m=s1，则返回1，如果m=s2，则返回2，如果m=sn，则返回n，说白了就是返回字符串m在字符串列表(s1,s2,,,,sn)的位置 索引从1开始 |
| FIELD(s,s1,s2,…,sn)               | 返回字符串s在字符串列表中第一次出现的位置                    |
| FIND_IN_SET(s1,s2)                | 返回字符串s1在字符串s2中出现的位置。其中，字符串s2是一个以逗号分隔的字符串 |
| REVERSE(s)                        | 返回s反转后的字符串 abc->cba                                 |
| NULLIF(value1,value2)             | 比较两个字符串，如果value1与value2相等，则返回NULL，否则返回value1 |



![image-20231122111115798](https://zhangwenkang666.oss-cn-beijing.aliyuncs.com/image-20231122111115798.png)
...
可以看到，ENCODE(value,password_seed)函数与DECODE(value,password_seed)函数互为反函数。DECODE(ENCODE(value,password_seed),password_seed) 进行解密



PASSWORD(str)，ENCODE(value,password_seed)，DECODE(value,password_seed)在mysql8.0中弃用
