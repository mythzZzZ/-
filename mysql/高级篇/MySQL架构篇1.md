...
![image-20231208162541211](https://zhangwenkang666.oss-cn-beijing.aliyuncs.com/image-20231208162541211.png)
...
* Optimizer: 查询优化器
  ...
  * SQL语句在语法解析之后、查询之前会使用==查询优化器==确定 SQL 语句的执行路径，生成一个 执行计划 。 
...
截止到现在，还没有真正去读写真实的表，仅仅只==是产出了一个执行计划==。于是就进入了执行器阶段 。
...
至此，这个语句就执行完成了。对于==有索引的表==，执行的逻辑也差不多。
...
### 2.5 oracle中的sql执行流程![image-20231230094739634](https://zhangwenkang666.oss-cn-beijing.aliyuncs.com/image-20231230094739634.png)



![image-20231230094850370](https://zhangwenkang666.oss-cn-beijing.aliyuncs.com/image-20231230094850370.png)
...
`InnoDB` 存储引擎是以页（**数据页**）为单位来管理存储空间的，我们进行的增删改查操作其实本质上都是在访问页面（包括读页面、写页面、创建新页面等操作）。而磁盘 I/O 需要消耗的时间很多，而在内存中进行操作，效率则会高很多，为了能让数据表或者索引中的数据随时被我们所用，DBMS 会申请`占用内存来作为数据缓冲池` ，在真正访问页面之前，需要把在磁盘上的页缓存到内存中的 Buffer Pool 之后才可以访问。
...
其次，**频次决定优先级顺序**。**因为缓冲池的大小是有限的，比如磁盘有 200G，但是内存只有 16G，缓冲池大小只有 1G，就无法将所有数据都加载到缓冲池里，这时就涉及到优先级顺序，会`优先对使用频次高的热数据进行加载，先加载调用频次高的 `。**
...
- 多个用户同时访问 要进行加锁，加锁会导致速度变低

![image-20231230100228066](https://zhangwenkang666.oss-cn-beijing.aliyuncs.com/image-20231230100228066.png)
...
在buffer pool中修改的数据，而在磁盘中没有修改的数据称为脏数据
...
![image-20231230104004173](https://zhangwenkang666.oss-cn-beijing.aliyuncs.com/image-20231230104004173.png)
...
![image-20231230104121878](https://zhangwenkang666.oss-cn-beijing.aliyuncs.com/image-20231230104121878.png)



### 4.5 CSV 引擎：存储数据时，以逗号分隔各个数据项

![image-20231230104140134](https://zhangwenkang666.oss-cn-beijing.aliyuncs.com/image-20231230104140134.png)

使用案例如下

```mysql
mysql> CREATE TABLE test (i INT NOT NULL, c CHAR(10) NOT NULL) ENGINE = CSV; #使用csv引擎插入数据
