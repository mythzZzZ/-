...
* **失败的（failed）**
  ...
  当事务处在 `活动的` 或者 部分提交的 状态时，可能遇到了某些错误（数据库自身的错误、操作系统 错误或者直接断电等）而无法继续执行，或者人为的==停止当前事务的执行==，我们就说该事务处在 失 败的 状态。
...
![image-20240108100754056](https://zhangwenkang666.oss-cn-beijing.aliyuncs.com/image-20240108100754056.png)
...
对于两个事务Session A、Session B，Session A `读取`了一个字段，然后 Session B `更新`了该字段且提交。 之后 Session A `再次读取` 同一个字段， `值就不同` 了。那就意味着发生了不可重复读。
...
（修改一定量的数据后才把数据刷入到磁盘中，这些修改后的数据先存放在一个文件夹里面（REDO日志），让后再把redo日志刷入磁盘）
...
![image-20240109095812772](https://zhangwenkang666.oss-cn-beijing.aliyuncs.com/image-20240109095812772.png)

- Undo 物理模型不会恢复到原来那样
- 假如之前新增了一条数据，刚好开辟了一个新的数据页（改数据页里面只有一条数据）
- 我们对这条数据进行rollback，数据会删掉，但是数据页还会保留
...
>
> * 在 `READ COMMITTED` 隔离级别下，一个事务在执行过程中每次执行SELECT操作时都会生成一 个ReadView（记录历史数据），ReadView的存在本身就保证了`事务不可以读取到未提交的事务所做的更改` ，也就是避免了脏读现象；
...
- ==幻读不好加锁==，A事物首先读取原数据库的3条记录，对这三条加锁了，此时b事物对数据库插入5条数据，此时因为是插入操作是加不了锁的，此时A在重新读取数据时，出现的还是幻读
...
- ==上面两点的意思==
- ==就是说在行数据添加共享和排他锁时，会自动在数据表上生成意向锁（共享锁/排他锁）==
...
假设事务A在teacher表添加了某一行的排他锁，并未提交，语句如下所示:
...
- 给id=5的数据加锁，但是此时id=5的数据不存在，此时给id=5加的锁是间隙锁，不同事物间可以重复加间隙锁
...
**gap锁的提出仅仅是为了防止插入幻影记录而提出的。**虽然有`共享gap锁`和`独占gap锁`这样的说法，但是它们起到的作用是相同的。而且如果对一条记录加了gap锁（不论是共享gap锁还是独占gap锁），==并不会限制其他事务对这条记录加记录锁或者继续加gap锁==（gap锁可以重复加）。
...
原理：数据带一个版本号字段，修改数据后把版本号一同修改。

- A，B事物同时修改数据， A，B获取了同一个版本号，B速度快一点修改了元素据，然后在元素据的位置把版本号也改了，所以A修改时
...
* `trx_id` ：每次一个事务对某条聚簇索引记录进行改动时，都会把该事务的 `事务id` 赋值给 `trx_id` 隐藏列。 ==记录最近一次更新的事物ID==
...
4. `low_limit_id` ，表示生成ReadView时系统中应该分配给下一个事务的 id 值。low_limit_id 是系 统最大的事务id值，这里要注意是系统中的事务id，需要区别于正在活跃的事务ID。(==系统下一个新建事物的id，当前最大事物id+1（是系统所有的事物id+1 不仅仅是活跃的，包括完成的==）)
...
![image-20240124141542636](https://zhangwenkang666.oss-cn-beijing.aliyuncs.com/image-20240124141542636.png)
...
之后，我们把 `事务id` 为 `10` 的事务提交一下，就像这样：4
