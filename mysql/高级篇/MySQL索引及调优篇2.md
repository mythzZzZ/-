...
==减少一次I/O的访问，可以让内存与硬盘交互的时间减少很多，天上一天地下一年==
...
一个B+树的节点其实可以分成好多层，规定最下边的那层，也就是存放我们用户记录的那层为第 0 层， 之后依次往上加。之前我们做了一个非常极端的假设：存放用户记录的页 最多存放3条记录 ，存放目录项 记录的页 最多存放4条记录 。其实真实环境中一个页存放的记录数量是非常大的，假设所有存放用户记录 的叶子节点代表的数据页可以存放 100条用户记录 ，所有存放目录项记录 
...
索引按照物理实现方式，索引可以分为 2 种：聚簇（聚集）和非聚簇（非聚集）索引。我们也==把非聚集 索引称为二级索引或者辅助索引。==

#### 1. 聚簇索引

- 主键索引是聚簇索引，索引即数据，数据即索引，索引存放在数据旁边，连在了一起
- b+树是聚簇索引
...
![image-20231230153551735](https://zhangwenkang666.oss-cn-beijing.aliyuncs.com/image-20231230153551735.png)

* 每当为某个表创建一个B+树索引（聚簇索引不是人为创建的，默认就有）的时候，都会为这个索引创建一个 `根结点` 页面。最开始表中没有数据的时候，每个B+树索引对应的 `根结点` 中即没有用户记录，也没有目录项记录。
* 随后向表中插入用户记录时，先把用户记录存储到这个`根节点` 中。
* 当根节点中的可用 `空间用完时` 继续插入记录，此时会将根节点中的所有记录复制到一个新分配的页，新出现的页作为根节点的子页，此时根节点就成为了目录页，子页就成为了数据页，此后再插入数据时就插入到数据页。当根节点充当目录页满时，在复制一份，根节点就成为了爷爷页，他儿子就变成了目录页。根节点的升级顺序：数据页-》目录页-》爷爷页  所以说根节点万年不动
...
我们知道B+树索引的内节点中目录项记录的内容是 `索引列 + 页号` 的搭配，但==是这个搭配对于二级索引来说有点不严谨==。还拿 index_demo 表为例，假设这个表中的数据是这样的：
...
- compact行格式
  ...
  - 记录头信息
    ...
    - record_type
    - heap_no （序号）
    - n_owned （当前小组最后一个成员用来记录 改组有多少个成员）
    - next_record
  - 记录的真实数据
...
![image-20231230194844307](https://zhangwenkang666.oss-cn-beijing.aliyuncs.com/image-20231230194844307.png)
...
![image-20231230194551652](https://zhangwenkang666.oss-cn-beijing.aliyuncs.com/image-20231230194551652.png)





- 每个数据页里面都会维护一个二分法槽，进入数据页的时候并不是直接遍历单链表来找数据，而是通过二分法的槽位，看要获取的数据在哪个分组，然后再这个分组里面进行遍历
- 对每五条数据进行一个分组

![image-20231230194152865](https://zhangwenkang666.oss-cn-beijing.aliyuncs.com/image-20231230194152865.png)
...
![image-20231230194137631](https://zhangwenkang666.oss-cn-beijing.aliyuncs.com/image-20231230194137631.png)
...
![image-20231231093607387](https://zhangwenkang666.oss-cn-beijing.aliyuncs.com/image-20231231093607387.png)

Compact格式中的变长字段长度列表是倒序存放的 如第一行 是   03 02 01为变长字段长度列表（01为第一个varchar字段中数据的长度 长度为1，02为第二个varchar字段中数据的长度，03为第3个varchar字段中数据的长度 长度为3）

![image-20231231095147406](https://zhangwenkang666.oss-cn-beijing.aliyuncs.com/image-20231231095147406.png)





![image-20231231093652942](https://zhangwenkang666.oss-cn-beijing.aliyuncs.com/image-20231231093652942.png)



NULL值列表

![image-20231231094011474](https://zhangwenkang666.oss-cn-beijing.aliyuncs.com/image-20231231094011474.png)

![image-20231231094055510](https://zhangwenkang666.oss-cn-beijing.aliyuncs.com/image-20231231094055510.png)



数据列中存在隐藏列

- 如果自己没定义主键，使用系统默认的主键 row_id

![image-20231231094258681](https://zhangwenkang666.oss-cn-beijing.aliyuncs.com/image-20231231094258681.png)



行溢出（一行数据占用的内存比16kb大，比一页还要多）



![image-20231231100307227](https://zhangwenkang666.oss-cn-beijing.aliyuncs.com/image-20231231100307227.png)

极限情况

![image-20231231100952173](https://zhangwenkang666.oss-cn-beijing.aliyuncs.com/image-20231231100952173.png)

**varchar需要额外的空间记录用了多少个字符和其他信息，所以varchar不能直接拉满65535个字节**

![image-20231231101032225](https://zhangwenkang666.oss-cn-beijing.aliyuncs.com/image-20231231101032225.png)



行溢出如何处理

![image-20231231101201751](https://zhangwenkang666.oss-cn-beijing.aliyuncs.com/image-20231231101201751.png)



不同行格式处理行溢出

- compressed和dynamic记录地址
- compact和redundant分不同的页处理

![image-20231231101335763](https://zhangwenkang666.oss-cn-beijing.aliyuncs.com/image-20231231101335763.png)
...
![image-20231231101603779](https://zhangwenkang666.oss-cn-beijing.aliyuncs.com/image-20231231101603779.png)



![image-20231231101637651](https://zhangwenkang666.oss-cn-beijing.aliyuncs.com/image-20231231101637651.png)

与compact的区别

![image-20231231101809391](https://zhangwenkang666.oss-cn-beijing.aliyuncs.com/image-20231231101809391.png)
...
![image-20231231115851846](https://zhangwenkang666.oss-cn-beijing.aliyuncs.com/image-20231231115851846.png)
...
![image-20231231115928513](https://zhangwenkang666.oss-cn-beijing.aliyuncs.com/image-20231231115928513.png)
...
![image-20231231160257438](https://zhangwenkang666.oss-cn-beijing.aliyuncs.com/image-20231231160257438.png)
...
![image-20231231160339572](https://zhangwenkang666.oss-cn-beijing.aliyuncs.com/image-20231231160339572.png)

![image-20231231160418247](https://zhangwenkang666.oss-cn-beijing.aliyuncs.com/image-20231231160418247.png)
...
索引就是让数据按照某种顺序进行存储或检索，因此当我们使用 GROUP BY 对数据进行分组查询，或者使用 ORDER BY 对数据进行排序的时候，就需要对分组或者排序的字段进行索引 。如果待排序的列有多个，那么可以在这些列上建立组合索引 。建立好索引后他们是紧挨着的，所以要获取他们时，速度会更快

![image-20231231164930604](https://zhangwenkang666.oss-cn-beijing.aliyuncs.com/image-20231231164930604.png)

sql执行顺序先group by 后order by  ,此时会使用 student_id在前的索引。student_id在前的单个索引，组合索引
...
然后再看下查询==优化器的成本==，实际上我们只需要检索一个页即可：
...
* DEPENDENT SUBQUERY
  ...
  ![image-20240101113317830](https://zhangwenkang666.oss-cn-beijing.aliyuncs.com/image-20240101113317830.png)
...
* DEPENDENT UNION
  ...
  ![image-20240101113337787](https://zhangwenkang666.oss-cn-beijing.aliyuncs.com/image-20240101113337787.png)

  - 第一个相关子查询DEPENDENT SUBQUERY
  - 其余相关子查询 DEPENDENT UNION
...
* DERIVED
  ...
  从执行计划中可以看出，id为2的记录就代表子查询的执行方式，它的select_type是DERIVED, 说明该子查询是以物化的方式执行的。id为1的记录代表外层查询，大家注意看它的table列显示的是derived2，表示该查询时针对将==派生表==物化之后的表进行查询的。

  派生表：(SELECT key1, count(*) as c FROM s1 GROUP BY key1) 把查询结果固定下来为一张表

![image-20240101113820053](https://zhangwenkang666.oss-cn-beijing.aliyuncs.com/image-20240101113820053.png)
...
* MATERIALIZED
  ...
  ![image-20240101114003850](https://zhangwenkang666.oss-cn-beijing.aliyuncs.com/image-20240101114003850.png)

  ![image-20240101114130611](https://zhangwenkang666.oss-cn-beijing.aliyuncs.com/image-20240101114130611.png)

  (SELECT key1 FROM s2)对查询结果转化为物化表

   WHERE key1 IN (SELECT key1 FROM s2) 物化之后与外层查询进行连接查询
...
* `index`
  ...
  上述查询中的所有列表中只有key_part2 一个列，而且搜索条件中也只有 key_part3 一个列，这两个列又恰好包含在联合索引idx_key_part这个索引中，可是搜索条件key_part3不能直接使用该索引进行`ref`和`range`方式的访问，只能扫描整个`idx_key_part`索引的记录，所以查询计划的`type`列的值就是`index`。
...
在MySQL建立联合索引时会遵守最佳左前缀原则（从左往右 左边的优先），即最左优先，在检索数据时从联合索引的最左边开始匹配。
...
* 不能，范围右边的列不能使用。比如：(<) (<=) (>) (>=) 和 between 等 classId>20是范围 范围条件失效
* 如果这种sql出现较多，应该建立：
...
![image-20240102154114652](https://zhangwenkang666.oss-cn-beijing.aliyuncs.com/image-20240102154114652.png)

==小表驱动大表==

- LEFT JOIN中两个表都没有索引时，优化器会自动优化，形成小表驱动大表的情况（效率更高）
- 若两表LEFT JOIN要建立一个索引，建立在右表（被驱动表）更加合适（但是优化器是会自动优化，看哪个是被驱动表，所以我们建立索引之前应该看看 那个表被优化成了被驱动表）
...
- 自动优化 哪个是小表 哪个是大表 和left join不一样
...
驱动表A加载数据到内存中与非驱动表B匹配时，加载A的数据是一块块，多条数据一起加载，这样更快
...
理解方式3：查询字段个数 +主键（select的个数）<=  索引字段的个数 （前提是查询字段是索引字段的子集）

​                  这种方式直接查索引就行了，就可以查到自己想要的内容，不用进行回表
...
```mysql
CREATE INDEX idx_age_name ON student(age, NAME);
# select id,age,NAME字段   查询字段个数 +主键（select的个数）<= 索引字段的个数 
...
好处: 不用再次回表，避免了回表时对磁盘随机IO的访问，造成访问时间变多
...
![image-20240104161807218](https://zhangwenkang666.oss-cn-beijing.aliyuncs.com/image-20240104161807218.png)

- ==对key1进行条件判断时，假设得到100个结果，它先不回表，然后将这100个结果进行 and右边的条件进行筛选。得到满足两个条件的结果后在进行回表，这个叫ICP==
...
4. 当SQL使用覆盖索引时，不支持ICP优化方法。因为这种情况下使用ICP不会减少I/O。（因为使用覆盖索引是不回表的，ICP是减少回表的IO）
5. 相关子查询的条件不能使用ICP
...
数据库的范式设计越高阶，夯余度就越低，同时高阶的范式一定符合低阶范式的要求，满足最低要求的范式是第一范式（1NF）。在第一范式的基础上进一步满足更多规范的要求称为第二范式（2NF），其余范式以此类推。==满足3说明1，2都满足了==
...
* 超键 ：对于球员表来说，超键就是包括==球员编号或者身份证号的任意组合==，比如（球员编号） （球员编号，姓名）（身份证号，年龄）等。 
* 候选键 ：就是最小的超键（只有一个），对于球员表来说，候选键就是（球员编号）或者（身份证号）。 
...
第一范式主要确保数据库中每个字段的值必须具有`原子性`，也就是说数据表中每个字段的值为`不可再次拆分`的最小数据单元。（列不可拆分）
...
(表中所有字段组合起来是唯一的，且组合成主键的字段也是唯一的 且==主键中的所有字段都被非主键字段完全依赖==，非主键字段中的每一个字段都要依赖主键字段中的所有字段（完全依赖）)
...
完全依赖（姓名要依赖球员编号和比赛编号才算==完全依赖==，但是姓名只依赖主键中的一个球员编号就行了，这个叫==部分依赖==）
...
* 存储 空间变大了 
* 一个表中字段做了修改，另一个表中冗余的字段也需要做同步修改，否则 数据不一致 （外键）
...
若一个关系达到了第三范式，并且它只有一个候选键，或者它的每个候选键都是单属性，则该关系自然达到BC范式。（==主键不能依赖候选键==）
